...
public class HomeController {
    ...
    private void loadReclamationsData() {
        ...
        try {
            ...
            for (Reclamation rec : reclamationService.voir()) {
                // Vérifier le statut de chaque réclamation
                String statut = rec.getStatut().toLowerCase();
                if (statut.equals("resolu")) {
                    resolvedCount++;
                } else if (statut.equals("ouvert") || statut.equals("en cours")) {
                    pendingCount++;
                }
                // Note: les réclamations avec statut "rejete" ne sont comptées ni comme en attente ni comme résolues
                }
            ...
            pendingReclamationsCount.setText(String.valueOf(pendingCount));
                ...
                animateCounter(pendingReclamationsCount, 0, pendingCount, 1500);
            }
            
            if (resolvedReclamationsCount != null) {
            resolvedReclamationsCount.setText(String.valueOf(resolvedCount));
                // Animation du compteur
                animateCounter(resolvedReclamationsCount, 0, resolvedCount, 1500);
            }
            
            System.out.println("Statistiques des réclamations mises à jour: " + pendingCount + " en attente, " + resolvedCount + " résolues");
        } catch (Exception e) {
    ...
    public void showReclamations() {
        try {
            ...
            if (MainController.getInstance() == null) {
                ...
                Scene currentScene = reclamationsButton.getScene();
                if (currentScene != null) {
                    currentScene.setRoot(view);
                } else {
                    // Si on ne peut pas obtenir la scène actuelle, créer une nouvelle fenêtre
        ...
        MainController.getInstance().showMyReclamations();
            ...
            System.out.println("Redirection vers la page des réclamations...");
        } catch (Exception e) {
            e.printStackTrace();
            ...
            alert.setContentText("Impossible d'accéder à la page des réclamations: " + e.getMessage());
            alert.showAndWait();
        }
    }
    ...
    public void showAvis() {
        try {
            // Vérifier si l'instance de MainController existe
            if (MainController.getInstance() == null) {
                // Si non, charger directement la vue des avis
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/avis_list.fxml"));
                Parent view = loader.load();
                
                // Utiliser la scène actuelle pour afficher la vue
                Scene currentScene = avisButton.getScene();
                if (currentScene != null) {
                    currentScene.setRoot(view);
                } else {
                    // Si on ne peut pas obtenir la scène actuelle, créer une nouvelle fenêtre
        ...
        MainController.getInstance().showAvisList();
            ...
            System.out.println("Redirection vers la page des avis...");
        } catch (Exception e) {
            e.printStackTrace();
            // Afficher une alerte en cas d'erreur
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Erreur de navigation");
            alert.setHeaderText(null);
            alert.setContentText("Impossible d'accéder à la page des avis: " + e.getMessage());
            alert.showAndWait();
        }
    }
