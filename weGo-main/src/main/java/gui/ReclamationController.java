...
public class ReclamationController {
    ...
    public void ajouterReclamation() {
        ...
        try {
            ...
            if (currentUser == null) {
                // Si non, créer un utilisateur par défaut
                ...
                System.out.println("Création d'un utilisateur par défaut pour la réclamation");
            }
            ...
            if (isEditMode && reclamationToEdit != null) {
                ...
                // S'assurer que l'utilisateur est défini même en mode édition
            ...
            if (categorieField.getText().isEmpty()) {
                ...
                reclamation.setPriorite(Integer.parseInt(prioriteField.getText()));
                    ...
                    // Utiliser une priorité par défaut
                    reclamation.setPriorite(3);
                }
            }
            
            // Vérification finale pour s'assurer que l'utilisateur est défini
            if (reclamation.getUser() == null) {
                reclamation.setUser(currentUser);
            }
            
            // Vérifier que tous les champs obligatoires sont définis
            if (reclamation.getTitre() == null || reclamation.getTitre().isEmpty()) {
            ...
            // Tous les champs sont valides, procéder à l'ajout/modification
            if (isEditMode) {
            ...
            if (isEditMode) {
                ...
                MainController.getInstance().showMyReclamations();
                    ...
                    // Fallback - simplement fermer la fenêtre actuelle si possible
                ...
                Alert choixNavigation = new Alert(AlertType.CONFIRMATION);
    ...
    public void backToHome() {
        try {
            ...
            if (MainController.getInstance() == null) {
                ...
                // Déterminer la scène actuelle en fonction du contexte
                Scene currentScene = null;
                Node sourceNode = null;
                
                if (reclamationTable != null) {
                    sourceNode = reclamationTable;
                } else if (reclamationsContainer != null) {
                    sourceNode = reclamationsContainer;
                } else if (titreField != null) {
                    sourceNode = titreField;
                } else if (BackHome != null) {
                    sourceNode = BackHome;
                }
                
                if (sourceNode != null) {
                    currentScene = sourceNode.getScene();
                }
                
                if (currentScene != null) {
                    currentScene.setRoot(view);
                } else {
                    // Si on ne peut pas obtenir la scène actuelle, essayer de trouver la fenêtre
                    Stage stage = null;
                    if (sourceNode != null) {
                    ...
                    if (stage != null) {
                        ...
                        throw new RuntimeException("Impossible de trouver une scène ou une fenêtre pour la navigation");
                    }
                }
            } else {
    ...
    public void backToReclamations() {
        try {
            // Vérifier si l'instance de MainController existe
            if (MainController.getInstance() == null) {
                // Si non, charger directement la vue des réclamations
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/my_reclamations.fxml"));
                Parent view = loader.load();
                
                // Essayer d'obtenir le contrôleur et d'initialiser l'utilisateur
        ...
        MainController.getInstance().showMyReclamations();
            ...
            System.out.println("Retour à la liste des réclamations...");
        } catch (Exception e) {
            e.printStackTrace();
            // Afficher une alerte en cas d'erreur
            Alert alert = new Alert(AlertType.ERROR);
            alert.setTitle("Erreur de navigation");
            alert.setHeaderText(null);
            alert.setContentText("Impossible d'accéder à la liste des réclamations: " + e.getMessage());
            alert.showAndWait();
        }
    }
