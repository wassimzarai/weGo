...
public class ConducteurService {
    ...
    // INSERT Method
    public void insert(Conducteur conducteur) throws SQLException {
        String sql = "INSERT INTO conducteur(id_voiture, disponibilite, date_depart, date_arret) VALUES (?, ?, ?, ?)";
        try (PreparedStatement pst = cnx.prepareStatement(sql)) {
            pst.setInt(1, conducteur.getIdVoiture());  // Set ID Voiture
            pst.setInt(2, conducteur.getDisponibilite());  // Use setInt for IntegerProperty (disponibilite is now IntegerProperty)
            pst.setObject(3, conducteur.getDateDepart());  // Use setObject for LocalDate (dateDepart)
            pst.setObject(4, conducteur.getDateArret());   // Use setObject for LocalDate (dateArret)
            pst.executeUpdate();
    ...
    public List<Conducteur> getAll() throws SQLException {
    ...
    public void addConducteur(Conducteur conducteur) throws SQLException {
