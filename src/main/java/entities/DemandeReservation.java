...
public class DemandeReservation {
    ...
    private Reservation reservationAssociee; // La réservation créée à partir de cette demande
    ...
    public void setReservationAssociee(Reservation reservationAssociee) {
        this.reservationAssociee = reservationAssociee;
    }
     ...
     */
    public boolean estCompatibleAvec(Trajet trajet) {
        // Vérifier les critères de base
        if (!trajet.getVilleDepart().equalsIgnoreCase(this.villeDepart) ||
            !trajet.getVilleArrivee().equalsIgnoreCase(this.villeArrivee) ||
            !trajet.getDate().equals(this.date) ||
            trajet.getNombrePlacesRestantes() < this.nombrePlaces ||
            trajet.getPrix() > this.prixMaximum) {
            return false;
        }
        
        // Vérifier les préférences
        if (this.besoinPlaceGrossBagages && !trajet.isAutoriseGrossBagages()) {
            return false;
        }
        
        if (!this.accepteFumeur && trajet.isAutoriseFumeur()) {
            return false;
        }
        
        if (!this.accepteAnimaux && trajet.isAutoriseAnimaux()) {
            return false;
        }
