...
public class DashboardController {
    ...
    public void initialize() {
        try {
            ...
            try {
                ...
                System.err.println("Erreur lors de la récupération de l'utilisateur courant: " + e.getMessage());
            ...
            if (recentReclamationsTable == null) {
                System.err.println("Erreur: TableView n'est pas correctement injecté");
            ...
            try {
                ...
                System.err.println("Erreur lors de l'initialisation des colonnes: " + e.getMessage());
            }
            ...
            Thread dataLoadingThread = new Thread(() -> {
                try {
                    ...
                    System.err.println("Thread de chargement interrompu: " + e.getMessage());
                    javafx.application.Platform.runLater(() -> {
                        showAlert("Erreur", "Une erreur est survenue lors du chargement: " + e.getMessage());
                    });
                } catch (Exception e) {
                    System.err.println("Erreur générale dans le thread de chargement: " + e.getMessage());
                    e.printStackTrace();
                }
            });
    ...
    private void loadData() {
        try {
            ...
            // Récupérer toutes les réclamations avec gestion d'erreurs
            try {
                allReclamations = reclamationService.voir();
                System.out.println("Réclamations chargées: " + allReclamations.size());
            } catch (Exception e) {
                System.err.println("Erreur lors du chargement des réclamations: " + e.getMessage());
                showAlert("Erreur de données", "Impossible de charger les réclamations: " + e.getMessage());
                // Continuer avec une liste vide plutôt que de planter
                allReclamations = new ArrayList<>();
            }
            
            // Récupérer tous les avis avec gestion d'erreurs
            try {
                allAvis = avisService.getAll();
                System.out.println("Avis chargés: " + allAvis.size());
            ...
            // Si aucune donnée n'a été trouvée, créer des données de démonstration
            ...
            FadeTransition fadeIn = new FadeTransition(Duration.millis(500), categoriesChart);
            ...
            System.err.println("Erreur critique lors du chargement des données: " + e.getMessage());
            e.printStackTrace();
            showAlert("Erreur critique", "Une erreur est survenue lors du chargement des données: " + e.getMessage());
        }
    }
    ...
    private void updateCounters() {
        try {
            ...
            // Calculer les compteurs seulement si la liste n'est pas null
            if (allReclamations != null) {
                total = allReclamations.size();
                    ...
                    if (reclamation != null) {
                        ...
                        if (statut != null) {
                            if (statut.equalsIgnoreCase("En attente") || 
                                statut.equalsIgnoreCase("En cours")) {
                                pending++;
                            } else if (statut.equalsIgnoreCase("Résolue")) {
                                resolved++;
                            }
                        }
                        
                        // Compter les réclamations urgentes
                        Integer priorite = reclamation.getPriorite();
                        if (priorite != null && priorite >= 3) {
                            urgent++;
                        }
                    }
                }
            ...
            if (totalReclamationsCount != null) totalReclamationsCount.setText(String.valueOf(total));
            if (pendingReclamationsCount != null) pendingReclamationsCount.setText(String.valueOf(pending));
            if (resolvedReclamationsCount != null) resolvedReclamationsCount.setText(String.valueOf(resolved));
            if (urgentReclamationsCount != null) urgentReclamationsCount.setText(String.valueOf(urgent));
            
            // Ajouter une animation pour l'actualisation
            animateCounterUpdate(totalReclamationsCount, 0, total);
            animateCounterUpdate(pendingReclamationsCount, 0, pending);
            animateCounterUpdate(resolvedReclamationsCount, 0, resolved);
            animateCounterUpdate(urgentReclamationsCount, 0, urgent);
            
        } catch (Exception e) {
            System.err.println("Erreur lors de la mise à jour des compteurs: " + e.getMessage());
            // Ne pas laisser l'erreur se propager plus haut
        }
    }
    
    // Amélioration de l'animation des compteurs pour gérer les cas null
    private void animateCounterUpdate(Label counterLabel, int oldValue, int newValue) {
        ...
        try {
            ...
            // Durée de l'animation en fonction de la différence
            int duration = Math.min(1000, Math.max(200, Math.abs(newValue - oldValue) * 50));
            
            KeyFrame keyFrame = new KeyFrame(Duration.millis(duration),
                new KeyValue(new SimpleIntegerProperty(oldValue), newValue));
            
            // Écouter les changements de valeur et mettre à jour le label
            SimpleIntegerProperty intProperty = new SimpleIntegerProperty(oldValue);
            intProperty.addListener((observable, oldVal, newVal) -> {
                counterLabel.setText(String.valueOf(newVal));
            });
            
            timeline.getKeyFrames().add(
                new KeyFrame(Duration.millis(duration),
                    event -> intProperty.set(newValue)
                )
            );
            
            timeline.play();
        } catch (Exception e) {
            // En cas d'erreur, simplement définir la valeur sans animation
            counterLabel.setText(String.valueOf(newValue));
            System.err.println("Erreur lors de l'animation du compteur: " + e.getMessage());
        }
    }
        ...
        if (allAvis != null && !allAvis.isEmpty()) {
            ...
            for (Avis avis : allAvis) {
                try {
                    ...
                    System.err.println("Impossible de convertir la note: " + avis.getRating());
                }
            }
    ...
    private void initializeMap() {
        try {
            ...
            mapBox.setStyle("-fx-background-color: #f5f5f5; -fx-border-color: #ddd; -fx-border-width: 1px; -fx-padding: 15px;");
            mapBox.setAlignment(javafx.geometry.Pos.CENTER);
            
            // Conteneur de chargement
            VBox loadingBox = new VBox(10);
            loadingBox.setAlignment(javafx.geometry.Pos.CENTER);
            Label loadingLabel = new Label("Chargement de la carte...");
            loadingLabel.setStyle("-fx-font-style: italic; -fx-text-fill: #666666;");
            ...
            // Afficher d'abord le message de chargement
            if (mapContainer != null) {
                mapContainer.getChildren().clear();
                mapContainer.getChildren().add(mapBox);
            }
            
            // Préparer le HTML pour la carte
            final String mapHTML = buildMapHTML();
            
            // S'assurer que nous sommes sur le thread JavaFX
            if (javafx.application.Platform.isFxApplicationThread()) {
                loadMapInWebView(mapBox, mapHTML);
            } else {
                // Si nous ne sommes pas sur le thread JavaFX, utiliser runLater
                javafx.application.Platform.runLater(() -> loadMapInWebView(mapBox, mapHTML));
            }
        } catch (Exception e) {
            System.err.println("Erreur générale lors de l'initialisation de la carte: " + e.getMessage());
            e.printStackTrace();
            ...
            if (mapContainer != null) {
                ...
                Label errorLabel = new Label("Erreur lors du chargement de la carte: " + e.getMessage());
    ...
    private void loadMapInWebView(VBox mapBox, String mapHTML) {
        try {
            ...
            webEngine.getLoadWorker().stateProperty().addListener((observable, oldValue, newValue) -> {
                if (newValue == Worker.State.SUCCEEDED) {
                    ...
                    try {
                        ...
                        System.err.println("Erreur après le chargement de la carte: " + e.getMessage());
                    }
                    ...
                    try {
                        ...
                        System.err.println("Erreur lors de l'affichage de l'erreur de chargement: " + e.getMessage());
                    }
                }
            });
            ...
            System.err.println("Erreur lors de la création de la WebView sur le thread JavaFX: " + e.getMessage());
            e.printStackTrace();
            ...
            try {
                ...
                System.err.println("Erreur critique lors de l'affichage de l'erreur: " + ex.getMessage());
            }
        }
    }
    ...
    private HBox createMapLegend() {
                    ...
                    legend.setPadding(new javafx.geometry.Insets(10, 0, 5, 0));
                    ...
                    for (String[] status : statusInfo) {
                        ...
                        colorRect.setFill(javafx.scene.paint.Color.web(status[1]));
                        colorRect.setStroke(javafx.scene.paint.Color.WHITE);
    ...
    public void refreshDashboard() {
        try {
            ...
            try {
                ...
                System.err.println("Erreur lors de la réinitialisation des graphiques: " + e.getMessage());
            }
            
            // Recharger toutes les données
            loadData();
            
            // Réappliquer les styles personnalisés
            try {
                applyCustomChartStyles();
            } catch (Exception e) {
                System.err.println("Erreur lors de l'application des styles: " + e.getMessage());
            ...
            // Animation pour restaurer l'opacité
            try {
                if (statusChart != null) {
                ...
                System.err.println("Erreur lors de l'animation: " + e.getMessage());
                
                // En cas d'erreur d'animation, restaurer l'opacité sans animation
            ...
            showAlert("Tableau de bord actualisé", "Les données du tableau de bord ont été actualisées avec succès.");
        } catch (Exception e) {
            System.err.println("Erreur lors de l'actualisation du tableau de bord: " + e.getMessage());
            e.printStackTrace();
